'use strict';
const canvas = document.querySelector("canvas#colors");
resizeCanvas();
let config = {
SIM_RESOLUTION: 128,
DYE_RESOLUTION: 256,
...
window.addEventListener('mousemove', e => {
  ...
  let posX = scaleByPixelRatio(e.clientX - canvasRect.left);
  let posY = scaleByPixelRatio(e.clientY - canvasRect.top);
...
window.addEventListener('touchstart', e => {
  ...
  for (let i = 0; i < touches.length; i++) {
    ...
    let posX = scaleByPixelRatio(touches[i].pageX - canvasRect.left);
    let posY = scaleByPixelRatio(touches[i].pageY - canvasTop);
